/* ===================================================
   ...
   --- FIXED + UPGRADED ---
   --- NEW SLAB ENGINE (End-Sem Decision System) ---
   =================================================== */
...
/* =============================================
   --- NEW SLAB ENGINE ---
   Slab-Wise Decision Engine for End-Sem Simulator

   Math:
     final_attended = A + (F - x)         where x = future misses (0..F)
     final_total    = T + F
     attendance %   = (A + F - x) / (T + F) * 100

   For a slab [lo, hi):
     lo ≤ ((A + F - x)/(T + F)) * 100 < hi
     => (A + F) - hi*(T+F)/100  <  x  ≤  (A + F) - lo*(T+F)/100

     x_max_in_slab = floor((A + F) - lo*(T+F)/100)
     x_min_in_slab = floor((A + F) - hi*(T+F)/100) + 1     (strict <)
       (top slab has no upper bound: x_min = 0)

     Clamp x to [0, F].

   Interpretations exposed:
     • maxMiss  = max future classes you can miss while landing in (or above) this slab's lower bound
     • mustAttend = min future classes you must attend to reach this slab (= F - maxMiss when reachable)
   ============================================= */
...
function slabMarks(slabId, type) {
  // Returns marks integer for the given slab+type, or null if not tracked.
  const t = (type || "DSC").toUpperCase();
  if (["SEC","PRACTICAL","BREAK","GE"].includes(t)) return null;
  const map = {
...
/**
 * Compute slab-wise analysis.
 * Returns array of slab objects with achievability + maxMiss + mustAttend + marks.
 */
function computeSlabAnalysis(A, T, F, type) {
  const finalTotal = T + F;
  if (F <= 0 || finalTotal <= 0) return null;

  // Best & worst possible final %
  const bestPct  = ((A + F) / finalTotal) * 100;       // miss 0
  const worstPct = (A / finalTotal) * 100;              // miss all F

  // Critical detection: even missing 1 future class drops below 67%
  // => attendance after missing 1 = (A + F - 1)/(T+F)*100
  const pctIfMissOne = ((A + F - 1) / finalTotal) * 100;
  const criticalNoMiss = (F >= 1) && (pctIfMissOne < 67);

  const results = SLABS.map(slab => {
    // x_max: floor((A + F) - lo*(T+F)/100)
    const rawMax = (A + F) - (slab.lo * finalTotal) / 100;
    let xMax = Math.floor(rawMax + 1e-9);

    // x_min: lower bound of x for being inside slab (strict < hi)
    let xMin;
    if (slab.hi === Infinity) {
      xMin = 0;
    } else {
    ...
    // Achievable if there's a valid integer x in [xMin, xMax] ∩ [0, F]
    const achievable = (xMax >= 0) && (xMin <= F) && (xMax >= xMin) &&
                       (bestPct >= slab.lo - 1e-9);

    // Verify: even if xMax is computed, achievability also requires
    // that bestPct meets slab.lo (i.e. attending ALL future classes
    // can actually lift attendance into slab).
    const slabReachable = bestPct >= slab.lo - 1e-9;

    let maxMiss = null;          // max misses while staying ≥ slab.lo (within slab band when achievable)
    let mustAttend = null;       // min attends required to reach this slab band

    if (achievable && slabReachable) {
      maxMiss = xMax;             // misses allowed while still inside this slab (or higher within band)
      mustAttend = F - xMax;      // attend at least this many to land in slab band
...
/* =============================================
   DECISION ENGINE (per-class quick insight)
   ============================================= */
...
/* =============================================
   --- UPDATED LOGIC ---
   END-SEM RESULT RENDER (Slab-Wise Decision Engine)
   ============================================= */
function renderEndSem(key) {
  ...
  // Critical state — even one miss tanks below 67%
      ...
      <div class="slab-analysis">
        ...
        <div class="slab-row critical">
          <div class="slab-head">
            ...
            <span class="slab-label">You cannot miss ANY class</span>
          </div>
  ...
  const slabRowsHTML = analysis.slabs.map(slab => {
    ...
    // Build human guidance
    let guidance = '';
    if (slab.mustAttend === 0) {
      guidance = `You're already in this slab — miss up to <b>${slab.maxMiss}</b> of ${F}.`;
    } else if (slab.mustAttend === F) {
      guidance = `Attend all <b>${F}</b> remaining classes (cannot miss any to stay here).`;
    } else {
      guidance = `Attend at least <b>${slab.mustAttend}</b> out of ${F} (miss up to <b>${slab.maxMiss}</b>).`;
    }
    ...
    return `
      <div class="slab-row ${slab.color}">
        ...
        <div class="slab-stats">
          <div class="slab-stat">
            <span class="slab-stat-val">${slab.maxMiss}</span>
            <span class="slab-stat-lbl">max miss</span>
          </div>
          <div class="slab-stat">
            <span class="slab-stat-val">${slab.mustAttend}</span>
            <span class="slab-stat-lbl">min attend</span>
          </div>
          <div class="slab-stat">
            <span class="slab-stat-val">${slab.marks !== null ? slab.marks : '—'}</span>
            <span class="slab-stat-lbl">marks</span>
          </div>
        </div>
  ...
  // Below-67% advisory row (always shown for context)
  const belowRowHTML = `

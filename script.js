/* ===================================================
   ...
   --- FIXED: Next Common Free Time logic ---
   =================================================== */
...
/* =============================================
   ...
   --- REWRITTEN ---
   State-based logic:
     1. Build merged busy timeline (union of both users' classes)
     2. Find the NEXT moment (strictly > now) where:
          - someone is BUSY just before that moment, AND
          - both are FREE at and after that moment
     3. If both already free now → return null (no card needed)
     4. If both have no remaining classes today → return null
   ============================================= */

/** Returns sorted, merged busy intervals [start, end] for the given day & user. */
...
/** Returns true if user is currently in a (non-break) class. */
function isUserBusyNow(profile, dayKey, nowMins) {
  const intervals = getBusyIntervals(profile, dayKey);
  return intervals.some(([s, e]) => nowMins >= s && nowMins < e);
}

/** Returns true if user has any class ending strictly after nowMins today. */
function hasClassesRemainingToday(profile, dayKey, nowMins) {
  const intervals = getBusyIntervals(profile, dayKey);
  return intervals.some(([s, e]) => e > nowMins);
}

/**
 * Build the union busy timeline for a day, merged across both users.
 * Returns sorted, merged busy intervals.
 */
...
/**
 * Find the next moment (strictly > nowMins) on the given day where
 * BOTH users transition from "at least one busy" → "both free".
 *
 * That is exactly the END of a combined-busy interval that ends after now.
 *
 * Returns the minute value, or null if no such transition exists today.
 */
function findNextBothFreeTransition(dayKey, nowMins) {
  ...
  for (const [s, e] of combined) {
    // We need the END of an interval that is strictly in the future
    // (so that BEFORE this minute someone is busy, AT this minute both free).
    if (e > nowMins) return e;
  }
  return null;
}
...
/**
 * Public entrypoint:
 * Returns a human-readable string for the next common free time,
 * OR null if the block should be hidden.
 */
function findNextCommonFreeTime() {
  ...
  const suhaniBusy = isUserBusyNow("suhani", today, now);
  const lakshBusy  = isUserBusyNow("laksh",  today, now);
  ...
  return `Today at ${minsToDisplay(transition)}`;
}

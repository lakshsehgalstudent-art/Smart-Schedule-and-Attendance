...
function migrateItem(item) {
  ...
  // FIXED: Migrate old lecture/tutorial/break tags to new type system
  if (item.tag) {
...
let detailSubjectKey = null; // current subject detail key
...
function renderSchedule(container, dayData, viewingToday, day) {
  ...
  dayData.forEach((item, index) => {
    ...
    // --- PART 4: Inline action buttons (NEW FEATURE) ---
    let actionsHTML = '';
    if (!isBreakCard) {
...
/* =============================================
   PART 4: QUICK MARK (inline card buttons)
   --- NEW FEATURE ---
   ============================================= */
...
function updateBusySection() {
  ...
  if (ft && row && text) { row.style.display="flex"; text.textContent=`Next free together: ${ft}`; }
  else if (row) row.style.display = "none";
}

/* =============================================
   ATTENDANCE ENGINE
   --- FIXED: Only count ✅ and ❌ actions ---
   ============================================= */
...
function calcMarks(pct, type) {
  ...
  else                  slabs = [[85,2],[75,1.6],[70,1.2],[67,0.8],[0,0]]; // VAC, AEC
  for (const [threshold, marks] of slabs) if (pct >= threshold) return marks;
  return 0;
}

/* --- FIXED: total = only attended + missed (not cancelled, not timetable count) --- */
...
/* =============================================
   DECISION ENGINE
   --- FIXED: uses real attendance counts ---
   ============================================= */
...
/* =============================================
   PART 6/7/8: END-SEM CALCULATOR PER SUBJECT
   --- NEW FEATURE ---
   ============================================= */
...
function renderSubjectDetail(stat) {
  ...
  // Build history sorted by day order
  ...
  container.innerHTML = `
    ...
    <div class="detail-section">
      ...
      <canvas id="attChart" class="graph-canvas" height="120"></canvas>
    </div>

    <!-- PART 8: End-Sem Calculator per subject -->
    <div class="detail-section">
...
/* =============================================
   PART 7: LIGHTWEIGHT GRAPHS (canvas, no libs)
   --- NEW FEATURE ---
   ============================================= */
function drawAttChart(stat) {
  const canvas = document.getElementById("attChart");
  if (!canvas) return;
  const { attended, total, pct, type } = stat;
  const missed    = total - attended;
  const cancelled = stat.items.filter(i => i.item.status === "cancelled").length;
  const unmarked  = stat.items.length - attended - missed - cancelled;
  ...
  // Draw donut
  const total2 = attended + missed + (cancelled || 0) + (unmarked||0);
  if (total2 > 0) {
    const slices = [
      { val:attended,  color:"#16a34a" },
      { val:missed,    color:"#dc2626" },
      { val:cancelled, color:"#d1d5db" },
      { val:unmarked,  color:"#e2e8f0" }
    ].filter(s => s.val > 0);
    let angle = -Math.PI / 2;
    slices.forEach(s => {
      const sweep = (s.val / total2) * 2 * Math.PI;
      ctx.beginPath();
      ctx.moveTo(pieCx, pieCy);
      ctx.arc(pieCx, pieCy, pieR, angle, angle + sweep);
      ctx.closePath();
    ...
    ctx.fill();
    // Center text
    ctx.fillStyle = pct >= 75 ? "#16a34a" : pct >= 67 ? "#d97706" : "#dc2626";
    ctx.font = `bold ${pieR * 0.55}px 'DM Serif Display', serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(total > 0 ? pct+"%" : "--", pieCx, pieCy);
  } else {
    ctx.beginPath();
    ctx.arc(pieCx, pieCy, pieR, 0, 2*Math.PI);
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("--", pieCx, pieCy);
  }

  // Legend (right side)
  const lx = pieCx + pieR + 20;
  const ly = pieCy - 40;
  const legendItems = [
    { label:"Attended",  val:attended,  color:"#16a34a" },
    { label:"Missed",    val:missed,    color:"#dc2626" },
    { label:"Cancelled", val:cancelled, color:"#d1d5db" },
    { label:"Pending",   val:unmarked,  color:"#e2e8f0" }
  ].filter(s => s.val > 0);
  ...
  legendItems.forEach((item, i) => {
    ...
    ctx.strokeRect(lx, y - 5, 10, 10);
    ctx.fillStyle = "#555577";
    ctx.font = `600 12px 'DM Sans', sans-serif`;
    ctx.fillText(`${item.label}: ${item.val}`, lx + 14, y);
  });

  // Slab marker lines (right of legend)
  const barX = lx;
  const barW = Math.min(W - lx - 12, 80);
  if (barW > 20 && total > 0) {
    const barY   = ly + legendItems.length * 22 + 10;
    const barH   = 8;
    ...
    ctx.fillStyle = pct>=75?"#16a34a":pct>=67?"#d97706":"#dc2626";
    ctx.beginPath();
    ctx.roundRect ? ctx.roundRect(barX, barY, fillW, barH, 4) : ctx.rect(barX, barY, fillW, barH);
    ctx.fill();
    // Slab markers
    [67,75,85].forEach(slab => {
      const mx = barX + (slab/100)*barW;
      ...
      ctx.textAlign = "center";
      ctx.fillText(slab+"%", mx, barY+barH+10);
    });
  }
}

/* =============================================
   PART 8: END-SEM RESULT RENDER
   --- NEW FEATURE ---
   ============================================= */
